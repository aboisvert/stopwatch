...
class StopwatchGroup(val name: String) {
  ...
  def get(name: String): Stopwatch = {
    if (!enabled) {
      return DisabledStopwatch
    }

    val existing = if (enableOnDemand) create(name) else _stats.get(name)
  ...
  def resetAll() {
    for (s <- _stats.values.toArray) { x: StopwatchStatisticImpl => x.reset() }
  }

  /** Enable a stopwatch. */
  def enable(name: String) = {
    val stats = create(name)
    stats.enabled = true
    stats
  }
  ...
  private def create(name: String): StopwatchStatisticImpl = synchronized {
    ifStats(name, { x => x }, { 
